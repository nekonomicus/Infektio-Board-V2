document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleEventTypeChange() {
        ...
        switch (selectedType) {
            case 'OP': eventDetailsLabel.textContent = 'OP Details:'; eventDetailsInput.placeholder = 'Beschreibung der Operation...'; eventDetailsInput.required = true; break;
            case 'Antibiotika Start': case 'Antibiotika Ende': eventDetailsLabel.textContent = 'Antibiotikum / Dosis:'; eventDetailsInput.placeholder = 'z.B. Co-Amoxicillin 1.2g iv'; eventDetailsInput.required = (selectedType === 'Antibiotika Start'); break;
            case 'Labor CRP': eventDetailsLabel.textContent = 'CRP Wert (mg/l):'; eventDetailsInput.placeholder = 'Numerischer Wert, z.B. 50.5'; eventDetailsInput.required = true; break;
            case 'Labor Lc': eventDetailsLabel.textContent = 'Lc Wert (G/l):'; eventDetailsInput.placeholder = 'Numerischer Wert, z.B. 12.3'; eventDetailsInput.required = true; break;
            case 'Probenentnahme': eventDetailsLabel.textContent = 'Zusätzliche Details zur Probe:'; eventDetailsInput.placeholder = '(Optional) z.B. Entnahmeort...'; eventDetailsInput.required = false; break;
            default: eventDetailsLabel.textContent = 'Details / Wert:'; eventDetailsInput.placeholder = 'Details...'; eventDetailsInput.required = false;
        }
    }

    function handleEventSubmit(e) {
        e.preventDefault();
        const eventType = eventTypeInput.value; const eventDate = eventDateInput.value; const eventDetails = eventDetailsInput.value.trim(); const probeArt = probeArtInput.value; const probeKeim = probeKeimInput.value.trim();
        if (!eventDate) { alert('Bitte Datum auswählen.'); return; } if (!eventType) { alert('Bitte Ereignistyp auswählen.'); return; }
        let newEvent = { id: Date.now(), date: eventDate, type: eventType };
        switch (eventType) {
            case 'OP': case 'Antibiotika Start': if (!eventDetails) { alert(`Bitte Details für ${eventType} eingeben.`); return; } newEvent.details = eventDetails; break;
            case 'Antibiotika Ende': newEvent.details = eventDetails; break;
            case 'Labor CRP': case 'Labor Lc': const numericValue = parseFloat(eventDetails); if (isNaN(numericValue)) { alert(`Bitte einen gültigen numerischen Wert für ${eventType} eingeben.`); return; } newEvent.details = numericValue.toString(); break;
            case 'Probenentnahme': if (!probeArt) { alert('Bitte Probenart auswählen.'); return; } if (!probeKeim) { alert('Bitte nachgewiesenen Keim eingeben (oder "Negativ").'); return; } newEvent.sampleType = probeArt; newEvent.germ = probeKeim; newEvent.details = eventDetails; break;
            default: console.error("Unbekannter Ereignistyp:", eventType); return;
        }
        events.push(newEvent); events.sort((a, b) => new Date(a.date) - new Date(b.date));
        saveData(); render(); eventForm.reset(); handleEventTypeChange(); eventDateInput.focus();
    }
    ...
    function handlePrintTimeline() {
        ...
        // Ensure timeline is rendered before printing
        renderTimeline();
        // Use a small delay to allow the browser to potentially finish rendering the SVG updates
        setTimeout(() => {
            document.body.classList.add('printing-timeline-only');
            window.print();
            // Remove class after print dialog is likely closed or cancelled
    ...
    // --- Local Storage ---
    const STORAGE_KEY_EVENTS = 'patientTimelineEvents_v10'; // Version bump for safety
    const STORAGE_KEY_PATIENT = 'patientTimelinePatientData_v10';
    ...
    function loadData() {
        try {
            ...
            else { events = events.filter(event => event && typeof event.date === 'string' && event.date.match(/^\d{4}-\d{2}-\d{2}$/)); events.forEach(event => { if (event.type === 'Probenentnahme') { event.sampleType = event.sampleType || ''; event.germ = event.germ || ''; }}); events.sort((a, b) => new Date(a.date) - new Date(b.date)); }
    ...
    function render() {
        ...
        try {
             ...
             console.error("Fehler renderTimeline:", e); timelineSvg.innerHTML = ''; timelineStatus.textContent = "Fehler beim Rendern der Timeline. Details in der Konsole (F12)."; timelineStatus.style.color = 'red'; timelineStatus.style.display = 'block'; timelineSvgWrapper.style.display = 'none';
        }
    }
    ...
    function renderEventList() {
        ...
        const typeMap = { 'OP': 'OP', 'Antibiotika Start': 'AB Start', 'Antibiotika Ende': 'AB Ende', 'Probenentnahme': 'Probe', 'Labor CRP': 'CRP', 'Labor Lc': 'Lc' };
        events.forEach(event => {
            const li = document.createElement('li'); const textSpan = document.createElement('span'); const displayType = typeMap[event.type] || event.type; let displayText = `${dateLocaleFormat(event.date)} - ${displayType}: `;
            switch(event.type) {
                case 'OP': case 'Antibiotika Start': case 'Antibiotika Ende': displayText += event.details || ''; break;
                case 'Labor CRP': displayText += `${event.details} mg/l`; break;
                case 'Labor Lc': displayText += `${event.details} G/l`; break;
                case 'Probenentnahme': displayText += `${event.sampleType || '?'} - Keim: ${event.germ || '?'} ${event.details ? `(${event.details})` : ''}`; break;
                 default: displayText += event.details || '';
            }
            textSpan.textContent = displayText; li.appendChild(textSpan);
    ...
    function renderSummaries() {
        opListElement.innerHTML = ''; probeListElement.innerHTML = ''; antibioticListElement.innerHTML = '';
        const ops = events.filter(e => e.type === 'OP'); const probes = events.filter(e => e.type === 'Probenentnahme'); const antibioticStarts = events.filter(e => e.type === 'Antibiotika Start').sort((a,b) => new Date(a.date) - new Date(b.date)); const antibioticEnds = events.filter(e => e.type === 'Antibiotika Ende').sort((a,b) => new Date(a.date) - new Date(b.date));
        if (ops.length === 0) opListElement.innerHTML = '<li>Keine OPs.</li>'; else ops.forEach(op => opListElement.innerHTML += `<li><span class="date">${dateLocaleFormat(op.date)}:</span> ${escapeHtml(op.details)}</li>`);
        if (probes.length === 0) probeListElement.innerHTML = '<li>Keine Proben.</li>'; else probes.forEach(probe => { probeListElement.innerHTML += `<li><span class="date">${dateLocaleFormat(probe.date)}:</span> Art: ${escapeHtml(probe.sampleType || '?')} - Keim: ${escapeHtml(probe.germ || '?')} ${probe.details ? `(${escapeHtml(probe.details)})` : ''}</li>`; });
        const antibiotics = []; let usedEndIds = new Set();
        antibioticStarts.forEach(start => { const startDrugName = start.details ? start.details.split(' ')[0].trim().toLowerCase() : null; let matchedEnd = null; const potentialEnds = antibioticEnds.filter(end => !usedEndIds.has(end.id) && new Date(end.date) >= new Date(start.date)).sort((a, b) => { const dateDiff = new Date(a.date) - new Date(b.date); if (dateDiff !== 0) return dateDiff; const aMatches = a.details && startDrugName && a.details.toLowerCase().includes(startDrugName); const bMatches = b.details && startDrugName && b.details.toLowerCase().includes(startDrugName); if (aMatches && !bMatches) return -1; if (!aMatches && bMatches) return 1; if (!a.details && b.details && !bMatches) return -1; if (a.details && !aMatches && !b.details) return 1; return 0; }); for (const end of potentialEnds) { const endDrugName = end.details ? end.details.split(' ')[0].trim().toLowerCase() : null; if ( (startDrugName && endDrugName && endDrugName === startDrugName) || !endDrugName || !startDrugName ) { matchedEnd = end; usedEndIds.add(matchedEnd.id); break; } } antibiotics.push({ start: start.date, end: matchedEnd ? matchedEnd.date : null, details: start.details || '' }); });
    ...
    // --- Timeline Rendering (Adjusted for Larger Fonts/Layout) ---
    function renderTimeline() {
        ...
        const antibioticRowHeight = 20; // Increased height per AB row
        const totalAntibioticHeight = Math.max(1, antibioticRowCount) * antibioticRowHeight;

        // --- Setup - Adjusted Row Heights & Margins ---
        const baseRowHeight = 30; // Increased base height for OP row
        const textRowHeight = 25; // Increased height for text-based rows
        const rowGap = 15;        // Increased gap between row types
        const verticalTextOffset = 12; // Base offset for text within rows (adjust based on font size)

        const opY = 0;
        const antibioticStartY = opY + baseRowHeight + rowGap; // Position ABs below OPs with gap
        const probeY = antibioticStartY + totalAntibioticHeight + rowGap; // Position Proben below ABs
        const laborY = probeY + textRowHeight + rowGap; // Position Labor below Proben

        const margin = { top: 50, right: 50, bottom: 70, left: 130 }; // Increased margins for labels/text
        ...
        // Attempt to make width more dynamic, but fall back to a large minimum
        const svgWidth = Math.max(1200, timelineSvgWrapper.clientWidth > 0 ? timelineSvgWrapper.clientWidth : 1200);
        const width = svgWidth - margin.left - margin.right;
        const height = dynamicHeight;

        // --- SVG Canvas ---
        timelineSvg.setAttribute('viewBox', `0 0 ${svgWidth} ${svgHeight}`);
        // Set explicit width/height attributes for consistent scaling, especially for print/screenshot
        timelineSvg.setAttribute('width', svgWidth);
        timelineSvg.setAttribute('height', svgHeight);
        // Remove conflicting style settings
        ...
        // Add slightly more padding, ensuring at least 2 days
        const datePaddingMs = Math.max(timeDiffMs * 0.03, 2 * 24 * 60 * 60 * 1000);
        if (timeDiffMs === 0) { // Handle single day case
        ...
        const tickInterval = calculateTickInterval(totalTime, width); let currentTickDate = getFirstTickDate(minDate, tickInterval.unit); let safety = 0; const tickLabelY = height + 25; // Increased distance for larger font
        const tickMarkY1 = height; const tickMarkY2 = height + 6; const gridLineY1 = 0; const gridLineY2 = height;
        ...
        const yLabels = [
            ...
            { y: antibioticStartY + totalAntibioticHeight / 2, text: 'Antibiotika' },
            { y: probeY + textRowHeight / 2, text: 'Probenentnahme' },
            { y: laborY + textRowHeight / 2, text: 'Labor' }
        ];
        yLabels.forEach(label => { const textElement = document.createElementNS(SVG_NS, "text"); textElement.setAttribute('x', -15); /* Further left */ textElement.setAttribute('y', label.y); textElement.setAttribute('class', 'axis-label'); textElement.setAttribute('dominant-baseline', 'middle'); textElement.textContent = label.text; chartGroup.appendChild(textElement); });

        // --- Draw Events ---
        try {
            // Draw Antibiotic Bars
            const barHeight = antibioticRowHeight * 0.7; // Slightly thicker bar visually
            assignedPeriods.forEach(period => {
                const xStart = xScale(period.startDate);
                const xEnd = xScale(period.endDate ? period.endDate : maxDate); // Extend running ABs to padded maxDate
                const barWidth = Math.max(1, xEnd - xStart);
                 // Calculate row Y position (center of the row)
                 const rowCenterY = antibioticStartY + period.row * antibioticRowHeight + antibioticRowHeight / 2;
                 const barY = rowCenterY - barHeight / 2; // Bar centered in its lane

                if (barWidth > 0 && xStart < width) {
                    const rect = document.createElementNS(SVG_NS, "rect"); rect.setAttribute('x', xStart); rect.setAttribute('y', barY); rect.setAttribute('width', barWidth); rect.setAttribute('height', barHeight); rect.setAttribute('class', 'antibiotic-bar'); rect.setAttribute('rx', 3); rect.setAttribute('ry', 3); chartGroup.appendChild(rect);
                    const label = document.createElementNS(SVG_NS, "text"); label.setAttribute('x', xStart + 6); label.setAttribute('y', rowCenterY); // Label vertically centered in row
                    label.setAttribute('class', 'bar-text-label');
                     // Try to fit drug name, shorten if needed
                     let drugName = period.details.split(' ')[0] || '(Unbekannt)';
                     // Basic check: if bar is very short, maybe just show initials or skip label?
                     if (barWidth < 30 && drugName.length > 3) {
                         drugName = drugName.substring(0,1) + '.'; // Abbreviate if very short bar
                     } else if (drugName.length > 15) { // Shorten long names slightly
                        drugName = drugName.substring(0, 14) + '…';
                     }
                     label.textContent = drugName;
                     // Only add label if it fits reasonably
                     if (barWidth > 10) { // Don't add text to tiny bars
                        chartGroup.appendChild(label);
                     }
                }
            });

            // Draw OPs, Probes, Labs as Text Labels with Offset Logic
            const textPositions = {}; // Track occupied vertical slots per day/row: key = "yBase,dateString" -> count
            const labelPadding = 5; // Horizontal padding for labels
            ...
            validEvents.forEach(event => {
                ...
                switch (event.type) {
                    case 'OP':
                        yBase = opY + baseRowHeight / 2;
                        labelClass = 'op-label';
                        markerSize = 8; // Slightly larger marker
                        labelText = event.details.length > 35 ? event.details.substring(0, 32) + '...' : event.details; // Allow slightly longer text
                        // Draw OP diamond marker
                         const opMarker = document.createElementNS(SVG_NS, "rect");
                         opMarker.setAttribute('x', x - markerSize / 2); opMarker.setAttribute('y', yBase - markerSize / 2); opMarker.setAttribute('width', markerSize); opMarker.setAttribute('height', markerSize); opMarker.setAttribute('class', 'op-marker marker-base'); opMarker.setAttribute('transform', `rotate(45 ${x} ${yBase})`); chartGroup.appendChild(opMarker);
                        break;

                    case 'Probenentnahme':
                        yBase = probeY + textRowHeight / 2;
                        labelClass = 'probe-label';
                        labelText = `${event.sampleType || '?'} (${event.germ || '?'})`;
                        labelText = labelText.length > 40 ? labelText.substring(0, 37) + '...' : labelText; // Allow longer text
                        break;

                    case 'Labor CRP':
                        yBase = laborY + textRowHeight / 2;
                    ...
                    case 'Antibiotika Start': case 'Antibiotika Ende': return;
                    default: return; // Ignore unknown types
                }

                if (labelText) {
                    // Vertical offset for overlaps
                     const posKey = `${Math.round(yBase)},${dateString}`;
                     const offsetIndex = textPositions[posKey] || 0;
                     const yOffset = offsetIndex * verticalTextOffset; // Offset by N pixels vertically
                     textPositions[posKey] = offsetIndex + 1; // Increment count for this slot

                     const finalY = yBase + yOffset;

                    const label = document.createElementNS(SVG_NS, "text");
                     // Position text slightly right of the date point (or marker for OP)
                     label.setAttribute('x', x + (markerSize / 2) + labelPadding); // Use markerSize (0 for non-OP)
                    label.setAttribute('y', finalY);
                    label.setAttribute('class', `timeline-text-label ${labelClass}`);
                    label.textContent = labelText;
                    chartGroup.appendChild(label);

                    // Optional: Draw a small line from date point to offset text if offset is significant
                    if (yOffset > 0) {
                        const line = document.createElementNS(SVG_NS, "line");
                        line.setAttribute('x1', x);
                        line.setAttribute('y1', yBase); // Originates from the base Y for that row
                        line.setAttribute('x2', x + labelPadding - 1); // Point towards start of text
    ...
    function assignAntibioticRows(periods) {
         const assignedPeriods = []; let maxRows = 0; periods.sort((a, b) => { const startDiff = a.startDate - b.startDate; if (startDiff !== 0) return startDiff; const endA = a.endDate ? a.endDate.getTime() : Infinity; const endB = b.endDate ? b.endDate.getTime() : Infinity; return endA - endB; }); const rowEnds = []; for (const period of periods) { let assignedRow = -1; for (let i = 0; i < rowEnds.length; i++) { if (period.startDate >= rowEnds[i]) { assignedRow = i; rowEnds[i] = period.endDate ? period.endDate.getTime() + (24*60*60*1000) : Infinity; /* Add day gap */ break; } } if (assignedRow === -1) { assignedRow = rowEnds.length; rowEnds.push(period.endDate ? period.endDate.getTime() + (24*60*60*1000) : Infinity); } assignedPeriods.push({ ...period, row: assignedRow }); maxRows = Math.max(maxRows, assignedRow + 1); } return { assignedPeriods, maxRows };
     }

    // --- Timeline Axis Helper Functions (Identical, but check calculateTickInterval logic) ---
    function calculateTickInterval(timeRangeMs, widthPx) {
        // Aim for more ticks if width allows, min 80px per tick approx
        const targetTickWidth = 100; // Increased target width per tick
        const maxTicks = Math.max(5, Math.floor(widthPx / targetTickWidth));
        const msPerDay = 86400000; const days = timeRangeMs / msPerDay;
        if (days <= 0) return { unit: 'day', step: 1 };
        // Adjust logic slightly for better granularity
        ...
        if (days <= maxTicks * 7) return { unit: 'day', step: 3 }; // Every 3 days
        if (days <= maxTicks * 10) return { unit: 'day', step: 5 }; // Every 5 days
        if (days <= maxTicks * 20) return { unit: 'week', step: 1 }; // Weekly
        if (days <= maxTicks * 50) return { unit: 'week', step: 2 }; // Bi-weekly
        if (days <= maxTicks * 100) return { unit: 'month', step: 1 }; // Monthly
        if (days <= maxTicks * 250) return { unit: 'month', step: 2 }; // Every 2 months
        if (days <= maxTicks * 500) return { unit: 'month', step: 3 }; // Quarterly
        return { unit: 'month', step: 6 }; // Semi-annually
    }
    function getFirstTickDate(minDate, unit) {
        const firstTick = new Date(minDate); firstTick.setUTCHours(0, 0, 0, 0);
        switch (unit) {
            case 'day': break; // Start at the beginning of the first day
            case 'week': // Start on the first Monday on or after minDate
                const dayOfWeek = (firstTick.getUTCDay() + 6) % 7; // 0=Monday, 6=Sunday
                if (dayOfWeek > 0) { firstTick.setUTCDate(firstTick.getUTCDate() + (7 - dayOfWeek));}
                // Ensure it's not before the actual minDate if minDate itself was a Monday
                if (firstTick < minDate) { firstTick.setUTCDate(firstTick.getUTCDate() + 7); }
                break;
            case 'month': // Start on the 1st of the month on or after minDate
                firstTick.setUTCDate(1);
                if (firstTick < minDate) { firstTick.setUTCMonth(firstTick.getUTCMonth() + 1); }
                break;
            default: break;
        }
        // Ensure the very first tick isn't pushed beyond the start if the start date aligns perfectly
        while (firstTick < minDate) {
             if (unit === 'day') firstTick.setUTCDate(firstTick.getUTCDate() + 1);
             else if (unit === 'week') firstTick.setUTCDate(firstTick.getUTCDate() + 7);
             else if (unit === 'month') {
                 firstTick.setUTCMonth(firstTick.getUTCMonth() + 1);
                 firstTick.setUTCDate(1); // Ensure it stays on the 1st
             }
    ...
    function formatTickDate(date, unit) {
        ...
        try {
            // Show Month/Year boundary changes more prominently
            if (unit === 'month' && date.getUTCDate() === 1) {
                 return date.toLocaleDateString(lang, monthYearFormatOptions);
            }
             // Default to Day/Month for finer granularities or non-boundary months
